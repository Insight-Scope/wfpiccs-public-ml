...
def main():
    ...
    print(f"Total papers in the dataframe: {assessment_datacenter.assessment_df.shape[0]}")
    ...
    assessment_dataframe["title_abstract"] = assessment_dataframe.apply(
        lambda row: text_preprocessor.clean(f"{row['title']} {row['abstract']}"), axis=1
    )
    
    # Create the project query (the search vector) based on the project settings
    cleaned_project_query = text_preprocessor.clean(' '.join([WFPICCS_GOAL_OF_PROJECT, WFPICCS_INCLUSION_CRITERIA_TA]))
    ...
    assessment_dataframe['stem_tf_idf_cosine_similarirty_score'] = assessment_dataframe['title_abstract'].apply(
    ...
    assessment_dataframe["Reviewer Response"] = assessment_dataframe[['stem_tf_idf_cosine_similarity_score', 'empty_abstract']].apply(
        lambda citation: classify_papers(
            similarity_score=citation['stem_tf_idf_cosine_similarirty_score'],
            empty_abstract=citation['empty_abstract']
        )
    )

    # Save the results to an excel file
    assessment_dataframe.to_excel("output.xlsx", index=False)
